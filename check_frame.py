...
DEFAULT_HZ        = 60        # monitor refresh rate (Hz) — fallback only
    ...
    for (phase, trial_id, stim_pair_id), frames in groups.items():
        for i in range(1, len(frames)):
            ...
            if curr["frame_idx"] <= prev["frame_idx"] and curr["frame_idx"] == 0:
                continue
            # skip response frames — response processing can cause artificially long intervals
            if curr["event_marker"] == "response":
                continue
...
def compute_stats(
    ...
    # per-trial interval statistics (skip segment boundaries and response frames)
    all_intervals = []
...
def resolve_hz(paths: List[Path], hz_override: Optional[float]) -> float:
    """Return Hz from --hz flag, session_metadata.json, or DEFAULT_HZ fallback."""
    if hz_override is not None:
        return hz_override
    for p in paths:
        for parent in p.parents:
            meta = parent / "session_metadata.json"
            if meta.exists():
                with open(meta, encoding="utf-8") as f:
                    data = json.load(f)
                rate = data.get("refresh_rate_hz")
                if rate:
                    print(f"  [INFO] Refresh rate {float(rate):.1f} Hz read from {meta}")
                    return float(rate)
    print(f"  [INFO] session_metadata.json not found – using {DEFAULT_HZ} Hz")
    return DEFAULT_HZ
...
def main() -> None:
    ...
    parser.add_argument("--hz",  type=float, default=None,
                        help=f"Monitor refresh rate in Hz (default: read from session_metadata.json, else {DEFAULT_HZ})")
    ...
    expected_ms = 1000.0 / resolve_hz(paths, args.hz)
