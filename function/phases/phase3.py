...
def run_phase3(
    ...
    while result["response"] is None and not result["timed_out"]:
        ...
        char1_color = get_char_color(state, "char1", mouse_pos, cfg.P3_CHAR1_POS)
        char2_color = get_char_color(state, "char2", mouse_pos, cfg.P3_CHAR2_POS)
        ...
        for pos_name, circle_data in circles.items():
            placed_char = state["placements"][pos_name] # 놓인 위치 추적
        ...
        # 배치된 글자를 최상위 레이어에 그림 — second selection 중에도 항상 보임
        # 단, 선택 중인 글자가 해당 위치에 hover 중이면 선택 글자가 위에 보이도록 skip
        for pos_name, circle_data in circles.items():
